...
## [2.6.1] — 2026-06-08
### Added
- **Живое обновление списка** — `PerformanceObserver` следит за новыми потоками; видео, запущенные/подгруженные **после** открытия панели, появляются автоматически (раньше требовалось переоткрыть панель)
- **Кнопка ↻ «Обновить список»** в шапке — ручной перескан
- **Детект отдельных файлов субтитров** (`.vtt`/`.srt`), которые плеер загружает при включении CC — отображаются как карточка 📄 VTT с кнопкой скачивания
### Fixed
- Видео, проигранные после открытия панели, не добавлялись в список

## [2.6.0] — 2026-06-08
### Added
- **YouTube: реальное скачивание субтитров** — распознавание страниц `youtube.com/watch` и `youtu.be`, извлечение дорожек (включая авто-CC) из `ytInitialPlayerResponse`, скачивание в `.vtt`. Раньше показывалась только заглушка «используйте yt-dlp»
- **Ссылка на источник** для HLS-карточек (на master m3u8)
### Fixed
- YouTube watch-страница показывала 0 видео (плеер там не в iframe) — теперь определяется
### Known limitation
- HLS-видео (Circle.so и т.п.) обнаруживаются через Performance API только **после начала воспроизведения** — непросмотренные/лениво-загружаемые ролики в списке не появятся, пока их поток не запрошен страницей
